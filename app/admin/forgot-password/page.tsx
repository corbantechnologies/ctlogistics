  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        const { error } = await adminAuthClient.forgetPassword({
          email,
        ...
        console.error("[ADMIN FRONTEND] Error caught during forgetPassword API call:", err);
      ...
      <div className="relative z-10 w-full max-w-sm space-y-8">
        ...
        <form onSubmit={handleSubmit} className="glass-card p-7 space-y-5">
          ...
          <label className="block">
            ...
            <input name="email" type="email" required placeholder="admin@ctdrive.co.ke" className="input-field" />
          </label>
        ...
        <p className="text-center text-xs text-white/20">
          Remember your password? <a href="/admin/login" className="text-amber-400/60 hover:text-amber-400">Back to Login →</a>
        </p>
      </div>
    </div>
  );
