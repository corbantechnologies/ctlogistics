...
export async function createAdminUser(formData: FormData) {
  try {
    ...
    if (!isFirstSetup) {
      ...
      if (!session) return { error: "Unauthorized: An admin account already exists. Please log in." };
    ...
    if (existing) return { error: `An account with email ${data.email} already exists. Please log in instead.` };
    ...
    return { error: error.message || "Failed to create admin account. Please try again." };
  }
}
...
export async function createPartnerUser(formData: FormData) {
  try {
    ...
    if (!parsed.success) {
      const fieldErrors = parsed.error.flatten().fieldErrors;
      const firstError = Object.values(fieldErrors).flat()[0] || "Invalid partner input data.";
      return { error: firstError };
    }
    ...
    return { error: error.message || "Failed to create partner user" };
  }
}
